...
public class TurboAnnotated extends java.applet.Applet {
	...
	static double dcomp,dfan,dturbin,dburner ;//physical dimensions
...
"rm" + i + ".gif") ;
	...
	public double getGama(double temp, int opt) {
		// Utility to get gamma or air as a function of temperature 
		...
		return(number) ;//temperature dependant value of gamma
	}

	public double getCp(double temp, int opt)  {
		// Utility to get cp or air as a function of temperature 
		double number,a,b,c,d ;
		/* BTU/R */
		a =  -4.4702130e-13;
		b =  -5.1286514e-10;
	...
	class Solver {
		...
		public void setDefaults() {
			...
			gamopt = 1 ;
			u0d = 0.0 ;
			altd = 0.0 ;//altitude = 0 ie on ground or at sea level
			throtl = 100. ;//max setting of the engine

			for (i=0; i<=19; ++i) {
				trat[i] = 1.0 ;//initial values for key design parameters
				tt[i]   = 518.6 ;//Temperature deg R
				prat[i] = 1.0 ;//pressure ratio 
				pt[i]   = 14.7 ;//ambient sea level air pressure
				eta[i]  = 1.0 ;// efficiency
			}
			tt[4] = tt4 = tt4d = 2500. ;//turbine inlet temperature 
			tt[7] = tt7 = tt7d = 2500. ;
			prat[3] = p3p2d = 8.0 ;//high pressure compressor pressure ratio
			prat[13] = p3fp2d = 2.0 ;
			byprat = 1.0 ;//bypass ratio of mass fan flow div. core flow
			abflag = 0 ;

			fueltype = 0 ;
			fhvd = fhv = 18600. ;// fuel heating value for jet A
			a2d = a2 = acore = 2.0 ;//area of core engine
			diameng = Math.sqrt(4.0 * a2d / 3.14159) ;
			acap = .9*a2 ;// design data for an engine
			a8rat = .35 ;// design data for an engine
			a8 = .7 ;// design data for an engine
			a8d = .40 ;// design data for an engine
			arsched = 0 ;
			afan = 2.0 ; // design data for an engine
			a4 = .418 ; //design data for an engine
			...
			u0min  = 0.0 ;   u0max = 1500.;// setting speed limits
			altmin = 0.0 ;   altmax = 60000. ;//setting altitude limits
			thrmin = 30;     thrmax = 100 ;// setting thrust limits
			etmin  = .5;     etmax  = 1.0 ;// setting efficiency limits
			cprmin = 1.0;   cprmax = 50.0 ;//setting comp. pres. Ratio limits
			bypmin = 0.0;   bypmax = 10.0 ;//setting fan bypass limits
			fprmin = 1.0;   fprmax = 2.0 ;//setting fan press ratio limits
			t4min = 1000.0;   t4max = 3200.0 ;//setting turb inlet temp limit
			t7min = 1000.0;   t7max = 4000.0 ;//setting ab exit temp limits
			a8min = 0.1;      a8max = 0.4 ;//setting ab exit area limits
			a2min = .001;       a2max = 50.;//setting engine area limits
		...
		public void myDesign() {
			/* setting design parameters for an engine */
			ensav = entype ;//engine type ie turbo fan, turbojet, ramjet
			absav = abflag ;
			flsav = fueltype ;//setting fuel type
			fhsav = fhvd/flconv ;//fuel heating value *conv is unit           //      conversion
			t4sav = tt4d/tconv ;//turbine inlet temp design parameter
			t7sav = tt7d/tconv ;//afterburner exit temp design parameter
			p3sav = p3p2d ;//compressor pressure ratio design parameter
			p3fsav = p3fp2d ;//fan pressure ratio design parameter
			bysav = byprat;//fan bypass ratio design parameter
			acsav = acore ;//core area inlet design parameter
			a2sav = a2d/aconv;
			a4sav = a4 ;//turbine nozzle inlet area design parameter
			a4psav = a4p;
			gamsav = gama ;//environmental parameter
			gamosav = gamopt;
			ptfsav = pt2flag ;
			et2sav = eta[2] ;//inlet efficiency design parameter
			pr2sav = prat[2];//inlet pressure recover design parameter
			pr4sav = prat[4];//combustor pressure ratio design parameter
			et3sav = eta[3];//compressor efficiency design parameter
			et4sav = eta[4];//combustion efficiency design parameter
			et5sav = eta[5];//turbine efficiency design parameter
			et7sav = eta[7];//nozzle efficiency design parameter
			et13sav = eta[13];//fan efficiency design parameter
			a8sav = a8d/aconv ; //nozzle exit area design parameter
			a8mxsav = a8max/aconv ;
			a8rtsav = a8rat ;

			u0mxsav = u0max/lconv2 ;//max speed ft/sec 
			u0sav = u0d/lconv2 ;
			altsav = altd/lconv1 ;
			arssav = arsched ;
		...
		public void loadCF6() {
			...
			entype = 2 ;//turbofan is entype 2
			abflag = 0 ;// no afterburner
			fueltype = 0;//fuel is jet A
			fhvd = fhv = 18600. ;// heating value btu/lb
			tt[4] = tt4 = tt4d = 2500. ;// set turb inlet temp 
			tt[7] = tt7 = tt7d = 2500. ;// set ab exit temp
			prat[3] = p3p2d = 21.86 ;// set hpt press ratio
			prat[13] = p3fp2d = 1.745 ;// set fan press ratio
			byprat = 3.3 ;//set fan bypass air flow / core flow ratio
			acore = 6.965 ;// set area of core engine
			afan = acore * (1.0 + byprat) ;//compute fan area
			a2d = a2 = afan ;
			diameng = Math.sqrt(4.0 * a2d / 3.14159) ;//compute eng dia
			a4 = .290 ; //set turbine nozzle area
			a4p = 1.131 ;// set engine parameter
			acap = .9*a2 ;
			gama = 1.4 ;
			gamopt = 1 ;
			pt2flag = 0 ;
			eta[2] = 1.0 ;//inlet pressure recovery
			prat[2] = 1.0 ;//inlet pressure ratio
			prat[4] = 1.0 ;// combustor pressure ratio, usuall <1
			eta[3] = .959 ;//compressor efficiency
			eta[4] = .984 ;// combustor burning efficiency
			eta[5] = .982 ;// turbine efficiency
			eta[7] = 1.0 ;
			eta[13] = 1.0 ;//fan inlet pressure recovery
			a8d = 2.436 ; //core engine exit area
			a8max = .35 ;
			a8rat = .35 ;

			u0max = 1500. ;//set max velocity
			u0d = 0.0 ;
			altd = 0.0 ;
			arsched = 0 ;

			/* also setting max limits for temp, weight, diameter, lenght and materials params for fan, compressor, burner, turbine..*/
			wtflag = 0; weight = 8229.;// set engine weight 8229 lb
		...
		public void loadJ85() {
			...
			entype = 0 ;//turbojet selected
			abflag = 0 ;//no afterburner
			fueltype = 0;
			fhvd = fhv = 18600. ; // set fuel as jet A
			tt[4] = tt4 = tt4d = 2260. ;//set max turb inlet temp
			tt[7] = tt7 = tt7d = 4000. ;//set max ab exit temp
			prat[3] = p3p2d = 8.3 ;//set compr press ratio
			prat[13] = p3fp2d = 1.0 ;//set fan press ratio
			byprat = 0.0 ;// no fan thus on byass air
			a2d = a2 = acore = 1.753 ;// set engine area
			diameng = Math.sqrt(4.0 * a2d / 3.14159) ;
			afan = acore * (1.0 + byprat) ;
			a4 = .323 ;// set turbine nozzle are
			a4p = .818 ;
			acap = .9*a2 ;
			gama = 1.4 ;
			gamopt = 1 ;
			pt2flag = 0 ;
			eta[2] = 1.0 ;//inlet recovery efficiency
			prat[2] = 1.0 ;//inlet pressure recovery
			prat[4] = .85 ;//combustor pressure ratio
			eta[3] = .822 ;//compressor efficiency
			eta[4] = .982 ;//  combustion efficiency
			eta[5] = .882;// turbine efficiency
			eta[7] = .97 ;//exit nozzle efficiency
			eta[13] = 1.0 ;//fan efficiency
			a8d = .818 ; 
			a8max = .467 ;
			a8rat = .467 ;

			u0max = 1500. ;//setting max speed ft/sec
			u0d = 0.0 ;
			altd = 0.0 ;
			arsched = 1 ;
		...
		public void loadF100() {
			...
			entype = 1 ;
			abflag = 1 ;//afterburner selected
			fueltype = 0;
			fhvd = fhv = 18600. ;//jet A fuel selected
			tt[4] = tt4 = tt4d = 2499. ;//max turbine inlet temperature
			tt[7] = tt7 = tt7d = 3905. ;//max afterburner exit temperature
			prat[3] = p3p2d = 20.04 ;//compressor pressure ratio
			prat[13] = p3fp2d = 1.745 ;//fan pressure ratio
			byprat = 0.0 ;// bypass ratio
			a2d = a2 = acore = 6.00 ;// area of core
			diameng = Math.sqrt(4.0 * a2d / 3.14159) ;
			afan = acore * (1.0 + byprat) ;
			a4 = .472 ;//turbine nozzle area
			a4p = 1.524 ;
			acap = .9*a2 ;
			gama = 1.4 ;
			gamopt = 1 ;
			pt2flag = 0 ;
			eta[2] = 1.0 ;//inlet recovery
			prat[2] = 1.0 ;//inlet pressure recovery
			prat[4] = 1.0 ;//combustor pressure ratio
			eta[3] = .959 ;//compressor efficiency
			eta[4] = .984 ;//combustion efficiency
			eta[5] = .982 ;//turbine efficiency
			eta[7] = .92 ;//exhaust nozzle efficiency
			eta[13] = 1.0 ;//fan efficiency
			a8d = 1.524 ; 
			a8max = .335 ;
			a8rat = .335 ;

			u0max = 1500. ;//max speed ft/sec
			u0d = 0.0 ;
			altd = 0.0 ;
			arsched = 0 ;

			wtflag = 0; weight = 3875.;//weight of engine
			minlt = 1; dinlt = 170.;  tinlt = 900.;
			mfan = 2;  dfan = 293.;   tfan = 1500.;
			mcomp = 2; dcomp = 293.; tcomp = 1500.;
			mburner = 4; dburner = 515.; tburner = 2500.;
		...
		public void loadRamj() {
			...
			entype = 3 ;//ramjet engine
			athsched = 1  ;
			aexsched = 1 ;
			arthd = .4 ;
			arexitd = 3.0 ;
			abflag = 0 ;//no afterburner
			fueltype = 0;//selected jet A as fuel
			fhvd = fhv = 18600. ;//fuel heating value btu/lb
			tt[4] = tt4 = tt4d = 4000. ;//design burner exit temperature
			t4max = 4500. ;//max burner exit temperature
			tt[7] = tt7 = tt7d = 4000. ;// design exhaust nozzle temperature
			prat[3] = p3p2d = 1.0 ;//compressor pressure ratio
			prat[13] = p3fp2d = 1.0 ;//fan pressure ratio
			byprat = 0.0 ;//bypass ratio
			a2d = a2 = acore = 1.753 ;
			diameng = Math.sqrt(4.0 * a2d / 3.14159) ;
			afan = acore * (1.0 + byprat) ;
			a4 = .323 ;//combustor exit area
			a4p = .818 ;
			acap = .9*a2 ;
			gama = 1.4 ;
			gamopt = 1 ;
			pt2flag = 0 ;//no power turbine
			eta[2] = 1.0 ;//inlet efficiency
			prat[2] = 1.0 ;//inlet pressure ratio
			prat[4] = 1.0 ;//combustor pressure ratio, usually <1.0
			eta[3] = 1.0 ;//compressor efficiency
			eta[4] = .982 ;//combustion efficiency, fraction of fuel heating //      value that shows up as sensible heat
			eta[5] = 1.0 ;//turbine efficiency
			eta[7] = 1.0 ;//nozzle efficiency
			eta[13] = 1.0 ;//fan efficiency
			a8 = a8d = 2.00 ; 
			a8max = 15. ;
			a8rat = 4.0 ;
			...
			u0max = 4500. ;//max speed ft/sec
			u0d = 2200.0 ;
			altd = 10000.0 ;
			arsched = 0 ;

			wtflag = 0; weight = 976.;//engine weight
		...
		public void getFreeStream() {//get freestream air data
			rgas = 1718. ;                /* ft2/sec2 R */
			if (inptype >= 2) {
				ps0 = ps0 * 144. ;//unusual use of unit conversion
			}
			if (inptype <= 1) {            /* input altitude */
				alt = altd / lconv1  ;
				if (alt < 36152. ) {
					ts0 = 518.6 - 3.56 * alt / 1000. ;//freestream air temp
					ps0 = 2116. * Math.pow(ts0/518.6, 5.256) ;//freestrm air pres
				}
				if (alt >= 36152. && alt <= 82345.) {   // Stratosphere
					ts0 = 389.98 ;//freestream air temp
					ps0 = 2116. * .2236 *// freestream air pressure
							Math.exp((36000.-alt)/(53.35*389.98)) ;
				}
				if (alt >= 82345.) {
					ts0 = 389.98 + 1.645 * (alt-82345)/1000. ;//freestrm air temp
					ps0 = 2116. *.02456 * Math.pow(ts0/389.98,-11.388) ;
				}//freestream air pressure
			}
			...
			if (inptype == 0 || inptype == 2) {           /* input speed  */
				...
				fsmach = u0/a0 ;// freestream mach number
				q0 = gama / 2.0*fsmach*fsmach*ps0 ;//freestream dynamic press
			}
			if (inptype == 1 || inptype == 3) {            /* input mach */
				u0 = fsmach * a0 ;
				u0d = u0 * lconv2 / 5280.* 3600. ;      /* airspeed ft/sec */
				q0 = gama / 2.0*fsmach*fsmach*ps0 ;/* dynamic press */
			}
			if (u0 > .0001) rho0 = q0 /(u0*u0) ;
			else rho0 = 1.0 ;

			tt[0] = ts0*(1.0 + .5 * (gama -1.0) * fsmach * fsmach) ;/*inlet total temp*/
			pt[0] = ps0 * Math.pow(tt[0]/ts0,gama/(gama-1.0)) ;/*inlet total pressure*/
			ps0 = ps0 / 144. ;// conversion lb/ft^2 to psi
			pt[0] = pt[0] / 144. ;// conversion lb/ft^2 to psi 
		...
		public void getThermo() {
			//all thermodynamics calculations are done in this routine
			...
			if (pt2flag == 0) {               /*     mil spec      */
				...
				eta[2] = prat[2] ;// inlet recovery efficiency
				fl1 = filter3(prat[2]) ;
			...
			trat[7] = 1.0 ;
			prat[7] = 1.0 ;
			tt[2] = tt[1] = tt[0] ;// inlet total temperature
			pt[1] = pt[0] ;// inlet total pressure
			gam[2] = getGama(tt[2],gamopt) ;// inlet air gamma
			cp[2]  = getCp(tt[2],gamopt);// inlet air specific heat
			pt[2] = pt[1] * prat[2] ;// inlet air pressure
			/* design - p3p2 specified - tt4 specified */
			if(inflag == 0) {

				if (entype <= 1) {              /* turbojet */
					prat[3] = p3p2d ;/* core compressor press ratio  */
					if (prat[3] < .5) prat[3] = .5 ;
					delhc = (cp[2]*tt[2]/eta[3])*
							(Math.pow(prat[3],(gam[2]-1.0)/gam[2])-1.0) ;
					deltc = delhc / cp[2] ;
					pt[3] = pt[2] * prat[3] ;// compressor exit pressure
					tt[3] = tt[2] + deltc ;//compressor exit temperature
					trat[3] = tt[3]/tt[2] ;// compressor exit/inlet temp ratio
					gam[3] = getGama(tt[3],gamopt) ;// air property at sta 3
					cp[3]  = getCp(tt[3],gamopt);//air property at sta 3
					tt[4] = tt4 * throtl/100.0 ;// turb inlet temp by throttle set
					gam[4] = getGama(tt[4],gamopt) ;//air property at sta4
					cp[4]  = getCp(tt[4],gamopt);//air property at sta4
					trat[4] = tt[4] / tt[3] ; //combustor temp ratio
					pt[4] = pt[3] * prat[4] ;// turbine inlet pressure
					delhht = delhc ;//turbine enthalpy drop
					deltht = delhht / cp[4] ;//turbine temperature drop
					tt[5] = tt[4] - deltht ;// turbine exit temperature
					gam[5] = getGama(tt[5],gamopt) ;//air property at turb exit
					cp[5]  = getCp(tt[5],gamopt);//air property at turb exit
					trat[5] = tt[5]/tt[4] ;//turbine temperature ratio
					prat[5] = Math.pow((1.0-delhht/cp[4]/tt[4]/eta[5]),
							(gam[4]/(gam[4]-1.0))) ;//turb exit pressure ratio
					pt[5] = pt[4] * prat[5] ;// turbine exit pressure
					/* fan conditions */
					prat[13] = 1.0 ;//fan pressure ratio
					trat[13] = 1.0 ;// fan temperature ratio
					tt[13]   = tt[2] ;// fan inlet temperature 
					pt[13]   = pt[2] ;//fan exit pressure
					gam[13]  = gam[2] ;// air property at fan exit
					cp[13]   = cp[2] ;// air property at fan exit
					prat[15] = 1.0 ;// fan nozzle pressure ratio
					pt[15]   = pt[5] ;// fan nozzle pressure
					trat[15] = 1.0 ;// fan nozzle temperature ratio
					tt[15]   = tt[5] ;// fan exit temperature
					gam[15]  = gam[5] ;// air property at fan exit
					cp[15]   = cp[5] ;// air property at fan exit
				}

				if(entype == 2) {                         /* turbofan */
					prat[13] = p3fp2d ;// fan pressure ratio
					if (prat[13] < .5) prat[13] = .5 ;
					delhf = (cp[2]*tt[2]/eta[13])*// fan air enthalpy gain
							(Math.pow(prat[13],(gam[2]-1.0)/gam[2])-1.0) ;
					deltf = delhf / cp[2] ;// fan air temperature gain
					tt[13] = tt[2] + deltf ;// fan air temperature calc
					pt[13] = pt[2] * prat[13] ;//fan exit pressure calc
					trat[13] = tt[13]/tt[2] ;// fan temperature ratio calc
					gam[13] = getGama(tt[13],gamopt) ;// air property at fan exit
					cp[13]  = getCp(tt[13],gamopt);// air property at fan exit
					prat[3] = p3p2d ;                      /* core compressor */
					if (prat[3] < .5) prat[3] = .5 ;
					delhc = (cp[13]*tt[13]/eta[3])*// compressor enthalpy gain
							(Math.pow(prat[3],(gam[13]-1.0)/gam[13])-1.0) ;
					deltc = delhc / cp[13] ;// air temperature gain in core compr
					tt[3] = tt[13] + deltc ;// compressor exit air temperature
					pt[3] = pt[13] * prat[3] ;//compressor exit pressure
					trat[3] = tt[3]/tt[13] ;//compressor exit/inlet temp ratio
					gam[3] = getGama(tt[3],gamopt) ;//air property at compr exit
					cp[3]  = getCp(tt[3],gamopt);//air property at compress exit
					tt[4] = tt4 * throtl/100.0 ;//turbine inlet temperature
					pt[4] = pt[3] * prat[4] ;//turbine inlet pressure
					gam[4] = getGama(tt[4],gamopt) ;//air property at turb inlet
					cp[4]  = getCp(tt[4],gamopt);//air property at turb inlet
					trat[4] = tt[4]/tt[3] ;//combustor temperature ratio
					delhht = delhc ;//equating hpt work to compressor work
					deltht = delhht / cp[4] ;// enthalpy drop in hpt
					tt[5] = tt[4] - deltht ;// temperature drop in hpt
					gam[5] = getGama(tt[5],gamopt) ;//air property at station 5
					cp[5]  = getCp(tt[5],gamopt);//air property at station 5
					trat[5] = tt[5]/tt[4] ;//hpt turbine temperature ratio
					prat[5] = Math.pow((1.0-delhht/cp[4]/tt[4]/eta[5]),
							(gam[4]/(gam[4]-1.0))) ;//hpt press ratio
					pt[5] = pt[4] * prat[5] ;//hpt exit pressure
					delhlt = (1.0 + byprat) * delhf ;//equating fan enthalpy to lpt
					deltlt = delhlt / cp[5] ;//lpt enthaly drop
					tt[15] = tt[5] - deltlt ;//fan exit nozzle temperature
					gam[15] = getGama(tt[15],gamopt) ;//air property at fan exit
					cp[15]  = getCp(tt[15],gamopt);//air property at fan exit
					trat[15] = tt[15]/tt[5] ;//fan nozzle temperature ratio
					prat[15] = Math.pow((1.0-delhlt/cp[5]/tt[5]/eta[5]),
							(gam[5]/(gam[5]-1.0))) ;//fan nozzle press ratio
					pt[15] = pt[5] * prat[15] ;
				}

				if (entype == 3) {              /* ramjet */
					prat[3] = 1.0 ; //no compressor thus compr press ratio is 1                     
					pt[3] = pt[2] * prat[3] ;
					tt[3] = tt[2] ;
					trat[3] = 1.0 ;
			...
			/* and nozzle throat ... then*/
				...
				if(entype == 2) {                       /*  turbofan */
					...
					while(Math.abs(dela) > .001 && itcount < 20) {
						...
						deriv = (delan-dela)/(t5t4n-trat[5]) ;
						dela = delan ;
						trat[5] = t5t4n ;//hpt exit temp div by inlet temp
						t5t4n = trat[5] - dela/deriv ;
					}
					tt[5] = tt[4] * trat[5] ;//
					gam[5] = getGama(tt[5],gamopt) ;
					...
					deltf =  - delhf / cp[2] ;//fan air temperature rise
					tt[13] = tt[2] + deltf ;// fan exit air temperature
					gam[13] = getGama(tt[13],gamopt) ;
					cp[13]  = getCp(tt[13],gamopt);
					trat[13] = tt[13]/tt[2] ;//fan temperature ratio
		...
		public void getPerform ()  {      /* determine engine performance */
			...
			rg1 = 53.3 ;// gas constant
			rg = cpair * (gama-1.0)/gama ;
			cp3 = getCp(tt[3],gamopt);                  /*BTU/lbm R */
			g0 = 32.2 ;// acceleration due to gravity ft per sec square
			ues = 0.0 ;
			game = getGama(tt[5],gamopt) ;//gamma at the exit plane of engine
			fac1 = (game - 1.0)/game ;
			cpe = getCp(tt[5],gamopt) ;//air property at exit of turbine
			...
			if (entype <=2) {    
				...
				pt[8] = epr*prat[2]*pt[0] ;// total air press at turb exit
				...
				if (npr <= 1.893) pexit = ps0 ;//not choked at exit
				else pexit = .52828 * pt[8] ;// choked at exit
				fgros = (uexit + (pexit - ps0) * 144. *  a8 /eair) / g0 ;
			}

			//turbo fan -- added terms for fan flow
			if (entype == 2) {
				fac1 = (gama - 1.0)/gama ;// used as exponent in thermo calcs 
				snpr = pt[13]/ps0;// fan exit press ratio
				...
				if (snpr <= 1.893) pfexit = ps0 ;//not choked fan operation
				else pfexit = .52828 * pt[13] ;// choked fan operation
			...
			//thrust in pounds
			fnlb = fnet * eair ;//eair is corrected air flow rate given M
			fglb = fgros * eair ;
			drlb = dram * eair ;
			...
			if (wtflag == 0) {
				if (entype == 0) {
					weight = .132 * Math.sqrt(acore*acore*acore) * 
							(dcomp * lcomp + dburner * lburn + dturbin * lturb + dnozl * lnoz);//simplistic treatment of weight calculations
				}
				if (entype == 1) {
	...
	class Con extends Panel {
		TurboAnnotated outerparent ;
		Up up ;
		Down down ;

		Con (TurboAnnotated target) { 
		...
		class Up extends Panel {
			...
			Up (TurboAnnotated target) { 
				outerparent = target ;
				setLayout(new GridLayout(3,2,5,5)) ;

				modch = new Choice() ;//mode selection
				modch.addItem("Design Mode") ;
				modch.addItem("Tunnel Test Mode");
				modch.addItem("Override");
				modch.select(0) ;

				engch = new Choice() ;//engine selection
				...
				pltch = new Choice() ;//plot selection
				...
				untch = new Choice() ;//units selection
			...
			public void handleProb(Object arg) {
				...
				if (plttyp != 7) {
					...
					// mode of operation of the java code
					...
					if(label.equals("Tunnel Test Mode")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.comPute() ;//compute thermodynamics of selected engine
						solve.myDesign() ;//upload myDesign
						ytrans = 115.0 ;
						view.start() ;
						varflag = 0 ;
						layin.show(in, "first")  ;
					...
					if(label.equals("Load My Design")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.loadMine() ;//load parameters of my design
					}
					if(label.equals("Load J85 Model")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.loadJ85() ;//load parameters of j85
					}
					if(label.equals("Load F100 Model")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.loadF100() ;//load parameters of F100
					}
					if(label.equals("Load CF6 Model")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.loadCF6() ;//load parameters of CF6
					}
					if(label.equals("Load Ramjet Model")) {
						...
						setUnits () ;
						untch.select(0) ;
						solve.loadRamj() ;//load parameters of RAMJET
					}
...
layout.show(out, "fourth")  ;
					 ...
					 */
					solve.comPute() ; //calculate performance for selected engine
				}
			} // end handler
		} // end up
		...
		public void setPanl() {
			...
			//  fan
			v1 = p3fp2d ;//fan pressure ratio
			vmn1 = fprmin;  vmx1 = fprmax ;
			v2 = eta[13] ;//fan efficiency
			vmn2 = etmin;  vmx2 = etmax ;
			v3 = byprat ;//fan bypass ratio
			...
			// compressor 
			v1 = p3p2d ;//compressor pressure ratio
			vmn1 = cprmin;  vmx1 = cprmax ;
			v2 = eta[3] ;//compressor efficiency
			vmn2 = etmin;  vmx2 = etmax ;
			...
			//  burner, this should be called combustor
			v1 = tt4d ;//combustor exit or turbine inlet temp
			vmn1 = t4min;  vmx1 = t4max ;
			v2 = eta[4] ;//combustion efficiency
			vmn2 = etmin;  vmx2 = etmax ;
			v3 = prat[4] ;//combustor pressure ratio
			vmn3 = etmin;  vmx3 = etmax ;
			v4 = fhvd ;//fuel heating value
	...
	class In extends Panel {
		...
		class Flight extends Panel {
			...
			class Right extends Panel {
				...
				Right (TurboAnnotated target) {
					...
					inptch = new Choice() ;//define flight modes of data input
				...
				public void handleBar(Event evt) {     // flight conditions
					...
					solve.comPute() ; //compute performance at flight condition
		...
		class Size extends Panel {
			...
			class Right extends Panel {
				...
				public void handleBar(Event evt) {     // engine size
					...
					solve.comPute() ; //calculate performance for designed engine
			...
			class Left extends Panel {
				...
				public void handleText(Event evt) {
					...
					solve.comPute() ;//calc performance for selected engine
		...
		class Inlet extends Panel {
			...
			class Right extends Panel {
				TurboAnnotated outerparent ;
				Scrollbar s1;
				Choice inltch,imat ;//inlet condition
				Label lmat;
				...
				Right (TurboAnnotated target) {
					...
					imat = new Choice() ;//material choices
				...
				public void handleMat(Event evt) {  // materials
					...
					case 1: dinlt = 170.7 ; tinlt = 900.; break ;//inlets
					...
					solve.comPute() ;//compute performance for selection
				}
		...
		class Burn extends Panel {
			...
			class Right extends Panel {
				...
				public void handleMat(Event evt) {
					...
					if (fueltype == 0) fhv = 18600. ;//jet A
					if (fueltype == 1) fhv = 49900. ;// hydrogen
...
if(entype == 3) displimg = anrmimg[counter-1] ;
		...
		public void handleb(int x, int y) {
			...
			if (y >= 27 && y <= 42 ) {   // set flight conditions 
				if (x >= 5   && x <= 40 ) {  
					layin.show(in, "first")  ;
					varflag = 0 ;
					solve.comPute () ; 
		...
		public void paint(Graphics g) {
			...
			if (inflag == 0) {
				...
				offsGg.drawString("TurboAnnotated Fan",200,25) ; 
				offsGg.setColor(Color.green) ;
				if (entype == 3) {
					offsGg.setColor(Color.yellow) ;
