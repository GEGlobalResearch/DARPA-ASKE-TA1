 ...
 * into the workspace.
 */
public class JavaFileImportWizard extends Wizard implements IImportWizard {
    private IWorkbench workbench;

    private IStructuredSelection selection;

    private JavaFileResourceImportPage1 mainPage;

    /**
     * Creates a wizard for importing resources into the workspace from
     * the file system.
     */
    public JavaFileImportWizard() {
        IDialogSettings workbenchSettings = WorkbenchPlugin.getDefault().getDialogSettings();
        IDialogSettings section = workbenchSettings
                .getSection("JavaFileImportWizard");//$NON-NLS-1$
        if (section == null) {
			section = workbenchSettings.addNewSection("JavaFileImportWizard");//$NON-NLS-1$
		}
    ...
    public void addPages() {
        super.addPages();
        mainPage = new JavaFileResourceImportPage1(workbench, selection);
        addPage(mainPage);
    }
    ...
    public void init(IWorkbench workbench, IStructuredSelection currentSelection) {
        ...
        setWindowTitle(AnswerImportMessages.DataTransfer_importTitle);
