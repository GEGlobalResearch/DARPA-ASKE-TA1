...
public abstract class CharacterReference extends Segment {
	 ...
	 * which is encoded as the numeric character reference "<code>&amp;#39;</code>" rather than its character entity reference {@link CharacterEntityReference#_apos &amp;apos;}
	 ...
	 * <p>
	 * Specifying a value of <code>true</code> as an argument to the <code>insideAttributeValue</code> parameter ensures that
	 * double quote characters (<code>"</code>) are encoded. The default behaviour is that they are not encoded if a value of <code>false</code> is specified.
	 * <p>
	...
	static Appendable appendEncode(final Appendable appendable, CharSequence unencodedText, final boolean insideAttributeValue) throws IOException {
		...
		for (int i=beginPos; i<endPos; i++) appendEncode(appendable,unencodedText.charAt(i),insideAttributeValue);
		return appendable;
	}
