...
public final class Config {
	...
	/**
	 * Determines whether apostrophes are encoded when calling the {@link CharacterReference#encode(CharSequence, boolean insideAttributeValue)} method.
	 * <p>
	 ...
	 * <p>
	 * This setting encodes a character if a {@linkplain CharacterEntityReference character entity reference} exists for the character,
	 * or the unicode code point is greater than U+007F.
	 * <p>
	 * The only exception to this is an {@linkplain CharacterEntityReference#_apos apostrophe} (U+0027),
	 * which is only encoded if the static {@link #IsApostropheEncoded} property is currently set to <code>true</code>.
	 * <p>
	 * The <code>insideAttributeValue</code> argument is ignored in this implementation.
	 * <p>
	 * To use this behaviour, set the static {@link #CurrentCharacterReferenceEncodingBehaviour} configuration variable to this constant
	 * during application initialisation. It cannot be configured differently for separate threads in the one application.
	 * <dl>
	 *  <dt>Example:</dt>
	 *  <dd><code>CharacterReference.CurrentCharacterReferenceEncodingBehaviour=CharacterReference.LEGACY_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR</code></dd>
	 * </dl>
	 */
	public static final CharacterReferenceEncodingBehaviour LEGACY_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR=new CharacterReferenceEncodingBehaviour() {
	 ...
	 * <ul>
	 *  <li>{@linkplain CharacterEntityReference#_amp ampersand} (<code>&amp;</code>)
	 *  <li>{@linkplain CharacterEntityReference#_lt left angle bracket} (<code>&lt;</code>)
	 *  <li>{@linkplain CharacterEntityReference#_gt right angle bracket} (<code>&gt;</code>)
	 *  <li>{@linkplain CharacterEntityReference#_quot double quote} (<code>"</code>), if the <code>insideAttributeValue</code> argument of the
	 *   {@link CharacterReference#encode(CharSequence, boolean insideAttributeValue)} method is set to <code>true</code>
	 *  <li>{@linkplain CharacterEntityReference#_apos apostrophe} (U+0027), if the static {@link Config#IsApostropheEncoded} property is currently set to <code>true</code>.
	 * </ul>
	 * <p>
	 * All other characters are left unencoded.
	 * <p>
	 * This default encoding behaviour only encodes the essential characters necessary to maintain valid HTML, assuming the encoding of the document is Unicode or
	 * some other encoding that can represent all of the characters in the document.
	 * <p>
	 * {@linkplain CharacterEntityReference#_gt Right angle bracket} (<code>&gt;</code>) characters are also encoded although technically they don't need to be.
	 ...
	 * <p>
	 * This default encoding behaviour is different to the default behaviour in parser versions 3.3 and earlier.
	 * The legacy behaviour assumed 7-bit ASCII encoding of the document which is now very uncommon, resulting in the unnecessary encoding of many characters.
	 * If your application is experiencing problems due to this change, consider using the {@linkplain #LEGACY_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR legacy behaviour} instead.
	 *
	 * @see CharacterReference#encode(CharSequence, boolean insideAttributeValue)
	 * @see #LEGACY_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR
	 */
	public static final CharacterReferenceEncodingBehaviour DEFAULT_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR=new CharacterReferenceEncodingBehaviour() {
		public boolean isEncoded(final char ch, final boolean insideAttributeValue) {
			return ch=='&' || ch=='<' || ch=='>' || (insideAttributeValue && ch=='"') || (ch=='\'' && Config.IsApostropheEncoded);
		}
	};

	/**
	 * Determines the currently active {@linkplain Config.CharacterReferenceEncodingBehaviour character reference encoding behaviour}.
	 * <p>
	 * This setting determines which characters are encoded when calling the {@link CharacterReference#encode(CharSequence, boolean insideAttributeValue)} method.
	 * <p>
	 * The default setting is {@link #DEFAULT_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR}.
	 * <p>
	 * Must not be <code>null</code>.
	 */
	...
	/**
	 * Specifies the interface for defining {@linkplain CharacterReference#encode(CharSequence,boolean) character reference encoding} behaviour.
	 * <p>
	 * The default implementation {@link Config#DEFAULT_CHARACTER_REFERENCE_ENCODING_BEHAVIOUR} is suitable for the vast majority of circumstances,
	 * so this interface is almost never used directly.
	 * <p>
	 * The currently active encoding behaviour is stored in the static {@link Config#CurrentCharacterReferenceEncodingBehaviour} property.
	 */
	public static interface CharacterReferenceEncodingBehaviour {
		/**
		 ...
		 * @return <code>true</code> if the specified character is to be encoded in the specified context, otherwise <code>false</code>.
		 */
